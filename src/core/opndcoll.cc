...
void opndcoll_rfu_t::init(unsigned num_banks, simt_core_ctx *shader) {
  ...
  for (unsigned j = 0; j < m_cu.size(); j++) {
    ...
    m_cu[j]->init(j, num_banks, m_bank_warp_shift, this,
...
bool opndcoll_rfu_t::writeback(warp_inst_t &inst) {
  ...
  for (unsigned op = 0; op < MAX_REG_OPERANDS; op++) {
    ...
    int reg_num = inst.out(op);  // this math needs to match that used
  ...
  for (unsigned i = 0; i < (unsigned)regs.size(); i++) {
    if (m_shader->get_config()->opu_clock_gated_reg_file) {
...
void opndcoll_rfu_t::dispatch_ready_cu() {
  for (unsigned p = 0; p < m_dispatch_units.size(); ++p) {
    ...
    if (cu) {
      ...
      for (unsigned i = 0; i < cu->get_num_operands(); i++) {
        if (m_shader->get_config()->opu_clock_gated_reg_file) {
          unsigned active_count = 0;
          for (unsigned i = 0; i < m_shader->get_config()->warp_size;
               i = i + m_shader->get_config()->n_regfile_gating_group) {
...
bool opndcoll_rfu_t::collector_unit_t::allocate(register_set *pipeline_reg_set,
  ...
  if ((pipeline_reg) and !((*pipeline_reg)->empty())) {
    ...
    for (unsigned op = 0; op < MAX_REG_OPERANDS; op++) {
      int reg_num =
          // (*pipeline_reg)->arch_reg.src[op];  // this math needs to match that used in
          (*pipeline_reg)->in(op);  // this math needs to match that used in
