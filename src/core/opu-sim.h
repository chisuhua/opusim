...
class opu_sim_config : public gpgpu_functional_sim_config {
 public:
...
class opu_sim : public OpuSimBase {
 public:
  opu_sim(const opu_sim_config &config, OpuContext *ctx, gem5::OpuTop *cuda_gpu = NULL );
...
class exec_opu_sim : public opu_sim {
 public:
  exec_opu_sim(const opu_sim_config &config, OpuContext *ctx, gem5::OpuTop *cuda_gpu = NULL )
      : opu_sim(config, ctx, cuda_gpu) {
    createSIMTCluster();
  }
