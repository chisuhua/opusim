...
unsigned int simtcore_config::max_cta(const kernel_info_t &k) const {
  ...
  if (result < 1) {
    printf(
        ...
        "has.\n");
    if (opu_ignore_resources_limitation) {
      printf(
          "GPGPU-Sim uArch: opu_ignore_resources_limitation is set, ignore "
          "the ERROR!\n");
      return 1;
    }
    abort();
...
void simtcore_config::reg_options(class OptionParser *opp) {
  ...
  option_parser_register(opp, "-opu_smem_latency", OPT_UINT32, &smem_latency,
                         "smem Latency", "3");
  /*
  option_parser_register(opp, "-opu_cache:dl1PrefL1", OPT_CSTR,
                         &m_L1D_config.m_config_stringPrefL1,
                         "per-shader L1 data cache config "
                         " {<nsets>:<bsize>:<assoc>,<rep>:<wr>:<alloc>:<wr_"
                         "alloc>,<mshr>:<N>:<merge>,<mq> | none}",
                         "none");
  option_parser_register(opp, "-opu_cache:dl1PrefShared", OPT_CSTR,
                         &m_L1D_config.m_config_stringPrefShared,
                         "per-shader L1 data cache config "
                         " {<nsets>:<bsize>:<assoc>,<rep>:<wr>:<alloc>:<wr_"
                         "alloc>,<mshr>:<N>:<merge>,<mq> | none}",
                         "none");
  option_parser_register(opp, "-opu_gmem_skip_L1D", OPT_BOOL, &gmem_skip_L1D,
                         "global memory access skip L1D cache (implements "
                         "-Xptxas -dlcm=cg, default=no skip)",
                         "0");
                         */

  option_parser_register(opp, "-opu_perfect_mem", OPT_BOOL,
                         &opu_perfect_mem,
  ...
  option_parser_register(
      opp, "-opu_clock_gated_reg_file", OPT_BOOL, &opu_clock_gated_reg_file,
      "enable clock gated reg file for power calculations", "0");
  option_parser_register(
      opp, "-opu_clock_gated_lanes", OPT_BOOL, &opu_clock_gated_lanes,
      "enable clock gated lanes for power calculations", "0");
  option_parser_register(opp, "-opu_shader_registers", OPT_UINT32,
                         &opu_shader_registers,
                         "Number of registers per shader core. Limits number "
                         "of concurrent CTAs. (default 8192)",
                         "8192");
  option_parser_register(
      opp, "-opu_registers_per_block", OPT_UINT32, &opu_registers_per_block,
      "Maximum number of registers per CTA. (default 8192)", "8192");
  option_parser_register(opp, "-opu_ignore_resources_limitation", OPT_BOOL,
                         &opu_ignore_resources_limitation,
                         "opu_ignore_resources_limitation (default 0)", "0");
  option_parser_register(
      opp, "-opu_shader_cta", OPT_UINT32, &max_cta_per_core,
      "Maximum number of concurrent CTAs in shader (default 8)", "8");
  option_parser_register(
      opp, "-opu_num_cta_barriers", OPT_UINT32, &max_barriers_per_cta,
      "Maximum number of named barriers per CTA (default 16)", "16");
  option_parser_register(opp, "-opu_n_clusters", OPT_UINT32, &n_simt_clusters,
                         "number of processing clusters", "10");
  option_parser_register(opp, "-opu_n_cores_per_cluster", OPT_UINT32,
                         &n_simt_cores_per_cluster,
                         "number of simd cores per cluster", "3");
  option_parser_register(opp, "-opu_n_cluster_ejection_buffer_size",
                         OPT_UINT32, &n_simt_ejection_buffer_size,
                         "number of packets in ejection buffer", "8");
  option_parser_register(
      opp, "-opu_n_ldst_response_buffer_size", OPT_UINT32,
      &ldst_unit_response_queue_size,
      "number of response packets in ld/st unit ejection buffer", "2");
  option_parser_register(
      opp, "-opu_shmem_per_block", OPT_UINT32, &opu_shmem_per_block,
      "Size of shared memory per thread block or CTA (default 48kB)", "49152");
  option_parser_register(
      opp, "-opu_shmem_size", OPT_UINT32, &opu_shmem_size,
      "Size of shared memory per shader core (default 16kB)", "16384");
  option_parser_register(opp, "-opu_shmem_option", OPT_CSTR,
                         &opu_shmem_option,
                         "Option list of shared memory sizes", "0");
  /*
  option_parser_register(
      opp, "-opu_unified_l1d_size", OPT_UINT32,
      &m_L1D_config.m_unified_cache_size,
      "Size of unified data cache(L1D + shared memory) in KB", "0");
  */
  option_parser_register(opp, "-opu_adaptive_cache_config", OPT_BOOL,
                         &adaptive_cache_config, "adaptive_cache_config", "0");
  option_parser_register(
      opp, "-opu_shmem_sizeDefault", OPT_UINT32, &opu_shmem_sizeDefault,
      "Size of shared memory per shader core (default 16kB)", "16384");
  option_parser_register(
      opp, "-opu_shmem_size_PrefL1", OPT_UINT32, &opu_shmem_sizePrefL1,
      "Size of shared memory per shader core (default 16kB)", "16384");
  option_parser_register(opp, "-opu_shmem_size_PrefShared", OPT_UINT32,
                         &opu_shmem_sizePrefShared,
                         "Size of shared memory per shader core (default 16kB)",
                         "16384");
  option_parser_register(
      opp, "-opu_shmem_num_banks", OPT_UINT32, &num_shmem_bank,
      "Number of banks in the shared memory in each shader core (default 16)",
      "16");
  option_parser_register(
      opp, "-opu_shmem_limited_broadcast", OPT_BOOL, &shmem_limited_broadcast,
      "Limit shared memory to do one broadcast per cycle (default on)", "1");
  option_parser_register(opp, "-opu_shmem_warp_parts", OPT_INT32,
                         &mem_warp_parts,
                         "Number of portions a warp is divided into for shared "
                         "memory bank conflict check ",
                         "2");
  option_parser_register(
      opp, "-opu_mem_unit_ports", OPT_INT32, &mem_unit_ports,
  ...
  option_parser_register(opp, "-opu_shmem_warp_parts", OPT_INT32,
                         ...
                         "2");
  option_parser_register(
      opp, "-opu_warpdistro_shader", OPT_INT32, &opu_warpdistro_shader,
      "Specify which shader core to collect the warp size distribution from",
      "-1");
  option_parser_register(
      opp, "-opu_warp_issue_shader", OPT_INT32, &opu_warp_issue_shader,
      "Specify which shader core to collect the warp issue distribution from",
      "0");
  option_parser_register(opp, "-opu_local_mem_map", OPT_BOOL,
                         &opu_local_mem_map,
                         "Mapping from local memory space address to simulated "
                         "GPU physical address space (default = enabled)",
                         "1");
  option_parser_register(opp, "-opu_num_reg_banks", OPT_INT32,
                         &opu_num_reg_banks,
                         "Number of register banks (default = 8)", "8");
  option_parser_register(
      opp, "-opu_reg_bank_use_warp_id", OPT_BOOL, &opu_reg_bank_use_warp_id,
      "Use warp ID in mapping registers to banks (default = off)", "0");
  option_parser_register(opp, "-opu_sub_core_model", OPT_BOOL,
                         &sub_core_model,
                         "Sub Core Volta/Pascal model (default = off)", "0");
  option_parser_register(opp, "-opu_enable_specialized_operand_collector",
                         OPT_BOOL, &enable_specialized_operand_collector,
                         "enable_specialized_operand_collector", "1");
  option_parser_register(opp, "-opu_operand_collector_num_units_sp",
                         OPT_INT32, &opu_operand_collector_num_units_sp,
                         "number of collector units (default = 4)", "4");
  option_parser_register(opp, "-opu_operand_collector_num_units_dp",
                         OPT_INT32, &opu_operand_collector_num_units_dp,
                         "number of collector units (default = 0)", "0");
  option_parser_register(opp, "-opu_operand_collector_num_units_sfu",
                         OPT_INT32, &opu_operand_collector_num_units_sfu,
                         "number of collector units (default = 4)", "4");
  option_parser_register(opp, "-opu_operand_collector_num_units_int",
                         OPT_INT32, &opu_operand_collector_num_units_int,
                         "number of collector units (default = 0)", "0");
  option_parser_register(opp, "-opu_operand_collector_num_units_tensor_core",
                         OPT_INT32,
                         &opu_operand_collector_num_units_tensor_core,
                         "number of collector units (default = 4)", "4");
  option_parser_register(opp, "-opu_operand_collector_num_units_mem",
                         OPT_INT32, &opu_operand_collector_num_units_mem,
                         "number of collector units (default = 2)", "2");
  option_parser_register(opp, "-opu_operand_collector_num_units_gen",
                         OPT_INT32, &opu_operand_collector_num_units_gen,
                         "number of collector units (default = 0)", "0");
  option_parser_register(opp, "-opu_operand_collector_num_in_ports_sp",
                         OPT_INT32, &opu_operand_collector_num_in_ports_sp,
                         "number of collector unit in ports (default = 1)",
                         "1");
  option_parser_register(opp, "-opu_operand_collector_num_in_ports_dp",
                         OPT_INT32, &opu_operand_collector_num_in_ports_dp,
                         "number of collector unit in ports (default = 0)",
                         "0");
  option_parser_register(opp, "-opu_operand_collector_num_in_ports_sfu",
                         OPT_INT32, &opu_operand_collector_num_in_ports_sfu,
                         "number of collector unit in ports (default = 1)",
                         "1");
  option_parser_register(opp, "-opu_operand_collector_num_in_ports_int",
                         OPT_INT32, &opu_operand_collector_num_in_ports_int,
                         "number of collector unit in ports (default = 0)",
                         "0");
  option_parser_register(
      opp, "-opu_operand_collector_num_in_ports_tensor_core", OPT_INT32,
      &opu_operand_collector_num_in_ports_tensor_core,
      "number of collector unit in ports (default = 1)", "1");
  option_parser_register(opp, "-opu_operand_collector_num_in_ports_mem",
                         OPT_INT32, &opu_operand_collector_num_in_ports_mem,
                         "number of collector unit in ports (default = 1)",
                         "1");
  option_parser_register(opp, "-opu_operand_collector_num_in_ports_gen",
                         OPT_INT32, &opu_operand_collector_num_in_ports_gen,
                         "number of collector unit in ports (default = 0)",
                         "0");
  option_parser_register(opp, "-opu_operand_collector_num_out_ports_sp",
                         OPT_INT32, &opu_operand_collector_num_out_ports_sp,
                         "number of collector unit in ports (default = 1)",
                         "1");
  option_parser_register(opp, "-opu_operand_collector_num_out_ports_dp",
                         OPT_INT32, &opu_operand_collector_num_out_ports_dp,
                         "number of collector unit in ports (default = 0)",
                         "0");
  option_parser_register(opp, "-opu_operand_collector_num_out_ports_sfu",
                         OPT_INT32, &opu_operand_collector_num_out_ports_sfu,
                         "number of collector unit in ports (default = 1)",
                         "1");
  option_parser_register(opp, "-opu_operand_collector_num_out_ports_int",
                         OPT_INT32, &opu_operand_collector_num_out_ports_int,
                         "number of collector unit in ports (default = 0)",
                         "0");
  option_parser_register(
      opp, "-opu_operand_collector_num_out_ports_tensor_core", OPT_INT32,
      &opu_operand_collector_num_out_ports_tensor_core,
      "number of collector unit in ports (default = 1)", "1");
  option_parser_register(opp, "-opu_operand_collector_num_out_ports_mem",
                         OPT_INT32, &opu_operand_collector_num_out_ports_mem,
                         "number of collector unit in ports (default = 1)",
                         "1");
  option_parser_register(opp, "-opu_operand_collector_num_out_ports_gen",
                         OPT_INT32, &opu_operand_collector_num_out_ports_gen,
                         "number of collector unit in ports (default = 0)",
                         "0");
  option_parser_register(opp, "-opu_coalesce_arch", OPT_INT32,
                         &opu_coalesce_arch,
                         "Coalescing arch (GT200 = 13, Fermi = 20)", "13");
  option_parser_register(opp, "-opu_num_sched_per_core", OPT_INT32,
                         &opu_num_sched_per_core,
                         "Number of warp schedulers per core", "1");
  option_parser_register(opp, "-opu_max_insn_issue_per_warp", OPT_INT32,
                         &opu_max_insn_issue_per_warp,
                         "Max number of instructions that can be issued per "
                         "warp in one cycle by scheduler (either 1 or 2)",
                         "2");
  option_parser_register(opp, "-opu_dual_issue_diff_exec_units", OPT_BOOL,
                         &opu_dual_issue_diff_exec_units,
  ...
  option_parser_register(opp, "-opu_simt_core_sim_order", OPT_INT32,
                         ...
                         "1");
  option_parser_register(
      opp, "-opu_pipeline_widths", OPT_CSTR, &pipeline_widths_string,
      "Pipeline widths "
  ...
  option_parser_register(opp, "-opu_tensor_core_avail", OPT_INT32,
                         &opu_tensor_core_avail,
                         "Tensor Core Available (default=0)", "0");
  option_parser_register(opp, "-opu_num_sp_units", OPT_INT32,
                         &opu_num_sp_units, "Number of SP units (default=1)",
                         "1");
  option_parser_register(opp, "-opu_num_dp_units", OPT_INT32,
                         &opu_num_dp_units, "Number of DP units (default=0)",
                         "0");
  option_parser_register(opp, "-opu_num_int_units", OPT_INT32,
                         &opu_num_int_units,
                         "Number of INT units (default=0)", "0");
  option_parser_register(opp, "-opu_num_sfu_units", OPT_INT32,
                         &opu_num_sfu_units, "Number of SF units (default=1)",
                         "1");
  option_parser_register(opp, "-opu_num_tensor_core_units", OPT_INT32,
                         &opu_num_tensor_core_units,
                         "Number of tensor_core units (default=1)", "0");
  option_parser_register(
      opp, "-opu_num_mem_units", OPT_INT32, &opu_num_mem_units,
      "Number if ldst units (default=1) WARNING: not hooked up to anything",
      "1");
  option_parser_register(
      opp, "-opu_scheduler", OPT_CSTR, &opu_scheduler_string,
  ...
  option_parser_register(
      opp, "-opu_concurrent_kernel_sm", OPT_BOOL, &opu_concurrent_kernel_sm,
      "Support concurrent kernels on a SM (default = disabled)", "0");
  option_parser_register(opp, "-opu_perfect_inst_const_cache", OPT_BOOL,
                         &perfect_inst_const_cache,
                         "perfect inst and const cache mode, so all inst and "
                         "const hits in the cache(default = disabled)",
                         "0");
  option_parser_register(
      opp, "-opu_inst_fetch_throughput", OPT_INT32, &inst_fetch_throughput,
      "the number of fetched intruction per warp each cycle", "1");
  option_parser_register(opp, "-opu_reg_file_port_throughput", OPT_INT32,
